    ...
    <div class="page-inner">
      ...
      <div v-if="showForm" class="form-card">
        ...
        <div class="field">
          ...
          <input v-model="form.titre" class="field-input" type="text" placeholder="Nom du parchemin" />
        </div>

        <div class="field">
          <label class="field-label">Titre du document <span class="field-hint">(affiché dans l'onglet du navigateur)</span></label>
          <input v-model="form.doc_titre" class="field-input" type="text" placeholder="Ex: Ordre Démoniaque — Parchemin Obscur" />
        </div>
...
onMounted(async () => {
  try { parchemins.value = await getMesParchemins() } catch {}
  loading.value = false
})
...
async function supprimerParchemin(p) {
  try {
    await deleteParchemin(p.token)
    parchemins.value = parchemins.value.filter(x => x.id !== p.id)
  } catch (e) { alert(e.message) }
}

function copierLien(token) {
  navigator.clipboard.writeText(`${window.location.origin}/parchemins/${token}`)
}
