    ...
    <div class="page-inner">
      ...
      <template v-else>
        ...
        <div class="projet-header">
          <h1 class="projet-titre">{{ parchemin.titre }}</h1>
          <p class="projet-info">
            Par <span class="projet-auteur">{{ parchemin.auteur_nom }}</span>
            · {{ formatDate(parchemin.created_at) }}
          </p>
        </div>

        <div class="projet-divider"></div>

        <BlocRenderer :blocs="parseBlocs(parchemin.contenu)" />

        <div class="projet-footer">
          <RouterLink v-if="isAuthor" to="/parchemins" class="back-link">← Mes parchemins</RouterLink>
...
import { getParchemin, deleteParchemin } from '../api.js'

const route = useRoute()
const router = useRouter()

const parchemin = ref(null)
const loading = ref(true)
const error = ref('')

const isAuthor = computed(() =>
  isLoggedIn.value && parchemin.value && currentUser.value?.id === parchemin.value.auteur_id
)

onMounted(async () => {
  try {
    parchemin.value = await getParchemin(route.params.token)
    if (parchemin.value?.doc_titre) {
      document.title = parchemin.value.doc_titre
    }
  } catch (e) {
...
async function supprimer() {
  if (!confirm('Supprimer ce parchemin ?')) return
  try {
    await deleteParchemin(parchemin.value.token)
    router.push('/parchemins')
  } catch (e) {
    alert(e.message)
  }
}
